...
async function startHisoka() {
    ...
    let pa7rick = await hisoka.sendContact(callerId, global.dono)
    hisoka.sendMessage(callerId, { text: `Sistem otomatis block!\nJangan menelpon bot!\nSilahkan Hubungi Dono Untuk Dibuka !`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       hisoka.send5ButImg(pea[0].id, `「 Alteração das configurações do grupo 」\n\nO grupo foi fechado pelo administrador, agora apenas o administrador pode enviar mensagens!`, `Mensagem de alteração de configurações do grupo`, wm_fatih, [])
       } else if(pea[0].announce == false) {
       hisoka.send5ButImg(pea[0].id, `「 Alteração das configurações do grupo 」\n\nO grupo foi aberto pelo administrador, agora os participantes podem enviar mensagens!`, `Mensagem de alteração de configurações do grupo`, wm_fatih, [])
       } else if (pea[0].restrict == true) {
       hisoka.send5ButImg(pea[0].id, `「 Alteração das configurações do grupo 」\n\nAs informações do grupo foram restritas, agora apenas o administrador pode editar as informações do grupo!`, `Mensagem de alteração de configurações do grupo`, wm_fatih, [])
       } else if (pea[0].restrict == false) {
       hisoka.send5ButImg(pea[0].id, `「 Alteração das configurações do grupo 」\n\nAs informações do grupo foram abertas, agora os participantes podem editar as informações do grupo!`, `Mensagem de alteração de configurações do grupo`, wm_fatih, [])
       } else {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Esse usuário foi banido ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await hisoka.getName(i + '@s.whatsapp.net')}\nFN:${await hisoka.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Contato\nitem2.EMAIL;type=INTERNET:https://discord.gg/MAjjb3U4Jz\nitem2.X-ABLabel:Discord\nitem3.URL:https://instagram.com/cak_haho\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Brasil;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	hisoka.sendMessage(jid, { contacts: { displayName: `${list.length} Contato`, contacts: list }, ...opts }, { quoted })
    }
